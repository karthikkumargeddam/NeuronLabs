...
async function runSeed() {
    console.log("Starting script to post 100+ advanced PhD labs to Strapi API...");
    let counter = 1000;
    let successCount = 0;
    ...
    let existingLabs = [];
    try {
        const checkRes = await fetch("http://localhost:1337/api/labs?pagination[limit]=500");
        const json = await checkRes.json();
        existingLabs = json.data || [];
    } catch (e) {
        console.error("Could not fetch existing labs. Is Strapi running? Error: " + e.message);
        return;
    }
    ...
    for (const topic of advancedTopics) {
        if (existingTitles.has(topic)) {
            console.log(`Lab "${topic}" already exists.`);
            continue;
        }
        ...
        const payload = {
            data: {
                name: "adv-lab-" + Date.now() + "-" + counter,
                uuid: "vbox-phd-cluster-" + Date.now() + "-" + counter,
        ...
        try {
            const res = await fetch("http://localhost:1337/api/labs", {
                method: "POST",
                headers: {
            ...
            if (!res.ok) {
                console.error("Failed to seed " + topic + ": " + res.statusText);
            } else {
                successCount++;
            }
        } catch (e) {
            console.error("Error pushing " + topic + ": " + e.message);
        }
        
        counter++;
    }
    console.log(`✅ Successfully seeded ${successCount} new advanced PhD labs via API!`);
}
